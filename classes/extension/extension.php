...
abstract class extension implements iextension {
    ...
    protected function get_mappings_by_mab(string $mabidentifier): array {
        ...
        // Get the extension start date. We only consider mappings created after this date.
        $extensionstartdate = strtotime(get_config('local_sitsgradepush', 'extension_start_date'));

        // Throw exception if the extension start date is not set.
        if (empty($extensionstartdate)) {
            throw new \moodle_exception('error:extensionstartdate', 'local_sitsgradepush');
        }
        ...
        $sql = "SELECT am.*
                ...
                AND am.timecreated > :extensionstartdate";
    ...
    protected function get_mappings_by_userid(int $userid): array {
        ...
        $sql = "SELECT am.*
                FROM {". manager::TABLE_ASSESSMENT_MAPPING ."} am
                JOIN {". manager::TABLE_COMPONENT_GRADE ."} mab ON am.componentgradeid = mab.id
                WHERE am.courseid $courseinsql AND am.moduletype $modinsql";
