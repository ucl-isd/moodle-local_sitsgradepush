...
class assign extends activity {
    ...
    protected function get_user_deadline_group_dates(int $userid): ?array {
        ...
        return [
            'startdate' => (int)($record->allowsubmissionsfromdate ?? $this->get_start_date()),
            'enddate' => (int)($record->duedate ?? $this->get_end_date()),
        ];
    }
    ...
    private function apply_user_date_extension(ec $extension, string $extensiontype): void {
        ...
        $this->overrides_due_date($newduedate, $extension->get_userid());

        // Get the assignment override record.
        $assignoverride = $this->get_override_record($extension->get_userid());
