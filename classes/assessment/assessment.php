...
abstract class assessment implements iassessment {
    ...
    /** @var array User IDs who have RAA overrides for this assessment */
    public array $raauserids = [];
    ...
    public function __construct(string $sourcetype, int $sourceid) {
        ...
        // Get RAA user IDs if extension is supported.
        if ($this->is_extension_supported()) {
            $this->raauserids = $this->get_users_with_raa_overrides();
        }
    }
    ...
    public function apply_extension(extension $extension): void {
        ...
        if ($extension instanceof ec) {
            ...
            if (!$extension->raarequiredprovisions->has_extension()) {
                // Remove RAA override if student has existing RAA override.
                if (in_array($extension->get_userid(), $this->raauserids)) {
    ...
    public function can_map_to_mab(int|\stdClass $mab, int $reassess): bool {
        ...
        if (!empty($mappings)) {
            foreach ($mappings as $mapping) {
                ...
                // Check this assessment does not map to a mab that has the same map code and period slot code.
                if (
                    $mapping->mapcode == $mab->mapcode
                    && $mapping->periodslotcode == $mab->periodslotcode
                ) {
                    return false;
                }
            }
