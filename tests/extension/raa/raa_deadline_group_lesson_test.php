...
/**
 * Tests for RAA extension with teacher-created deadline group overrides for lesson.
 *
 ...
 */
final class raa_deadline_group_lesson_test extends raa_base {
    /** @var \stdClass The ED03 test lesson. */
    ...
    public function setUp(): void {
        ...
        // Create ED03 lesson available: 13:00, deadline=15:30, timelimit=3600.
        ...
        $this->lesson = $this->create_lesson(
            $this->course1->id,
            $startdate,
            $enddate,
            HOURSECS
     ...
     * Student 1 is in DLG-A (earlier deadline), student 2 is in DLG-B (later available).
     * Each should be placed in a separate RAA group override reflecting their respective DLG deadline.
     *
     * @covers \local_sitsgradepush\assessment\lesson::get_user_deadline_group_dates
    ...
    public function test_students_in_different_dlg_groups_get_correct_extensions(): void {
        ...
        // DLG-A: deadline=14:10 for student 1 (TIER1, +20 min/hr on 1hr timelimit = +20 min).
        // DLG-B: available=14:20 for student 2 (TIER2, +25 min/hr on 1hr timelimit = +25 min).
        // DLG-B has no deadline override, so student 2's RAA base falls back to the original deadline (15:30).
        $dlgadeadline = (new DateTimeImmutable('2025-02-11 14:10:00'))->getTimestamp();
        $dlgbavailable = (new DateTimeImmutable('2025-02-11 14:20:00'))->getTimestamp();
        ...
        $this->create_lesson_group_override($this->lesson->id, $groupa, null, $dlgadeadline);
        $this->create_lesson_group_override($this->lesson->id, $groupb, $dlgbavailable, null);
        ...
        // Student 1: DLG-A deadline (14:10) + 20 min = 14:30.
        ...
        $override1 = $DB->get_record('lesson_overrides', [
        ...
        $this->assertNotEmpty(
            $override1,
            'Lesson override for student 1 RAA group should exist.'
        );
        // Student 1: timelimit 1hr + 20 min = 80 min = 4800 secs.
        $this->assertEquals(
            $this->lesson->timelimit + 20 * MINSECS,
            $override1->timelimit
        );
        $this->assertEquals($expecteda, $override1->deadline);

        // Student 2: original deadline (15:30, DLG-B has no deadline override) + 25 min = 15:55.
        ...
        $override2 = $DB->get_record('lesson_overrides', [
            'lessonid' => $this->lesson->id,
            'userid' => null,
            'groupid' => $raagroupid2,
        ]);
        $this->assertNotEmpty(
            $override2,
            'Lesson override for student 2 RAA group should exist.'
        );
        // Student 2: timelimit 1hr + 25 min = 85 min = 5100 secs.
        $this->assertEquals(
            $this->lesson->timelimit + 25 * MINSECS,
            $override2->timelimit
        );
        $this->assertEquals($expectedb, $override2->deadline);
    }
}
